...
- FFF grep gives line numbers and byte offsets, but hashline anchors cannot be computed safely from returned snippets alone. Grep may map anchors from a fresh stat-validated hash snapshot, from a full-file `lineHashes(fullContent, absolutePath)` fallback, or from an eligible no-snapshot pure-prefix hash path.
...
- Cold-cache prefix hashing is accepted for v1 by default, but only for files with no existing full hash-store snapshot. Prefix hashes are a separate partial cache, not a normal persistent snapshot: cache the file snapshot id, prefix end, and prefix hashes; if a later request needs more lines than cached, recompute from the beginning through the larger prefix and replace the prefix cache. Prefix cache entries must be invalidated when `fileSnap().snapshotId` changes, and removed once a full `read`, full-load grep fallback, or `replace` creates a complete hash-store snapshot for that file. If any full snapshot exists but stat metadata is stale or missing, grep must full-load and let `lineHashes(fullContent, path)` decide whether stored hashes can be preserved.
...
- Format grep match and context rows as pure `HASH│content` rows using anchors produced by a fresh stat-validated hash snapshot, full-file `lineHashes(fullContent, absolutePath)`, or an eligible no-snapshot pure-prefix hash computation. Displayed line content may be bounded/truncated with an explicit marker.
...
- On true cold files with no existing full hash-store snapshot, use prefix hashing by default through the last displayed context line. Cache partial prefix hashes separately from the persistent full hash-store; if a later request needs beyond the cached prefix, recompute from the file start to the larger prefix rather than trying to append incrementally. Do not write partial prefix hashes to the persistent hash store, and discard them once a complete snapshot exists.
...
- Reuse `pi-hashline` text loading and validation behavior for full-load paths so binary/image/oversized files do not emit hashline rows. Prefix hashing must require FFF `isBinary === false`, local stat size within `MAX_BYTES`, matching `fileSnap().snapshotId`, prefix/window text validation, and full-load fallback when those checks are insufficient. V1 should avoid extra profitability heuristics for prefix hashing beyond the full-snapshot/stale/validation rules.
