...
7. Add a grep module that registers `grep` and reuses the existing hashline helpers:
   ...
   - when no full snapshot exists, use the default prefix-hash path if the needed prefix is not already covered; reuse the partial prefix cache when it is covered;
...
- Cold-cache prefix hashing is in v1 scope by default only for files with no existing full hash-store snapshot. Prefix cache is partial and separate: reuse it only when the requested prefix is covered; otherwise recompute from the beginning through the larger prefix. Stale snapshots, metadata-less snapshots, or uncertain validation must fall back to full-file `lineHashes(content, path)`.
