...
- Write those comments alongside the implementation, not as a later documentation pass. New or changed public contracts, persistence and migration steps, concurrency or cancellation ownership, non-obvious validation, and intentional fallback behavior require concise comments that state the reason and invariant. Do not add narration for self-evident code.
