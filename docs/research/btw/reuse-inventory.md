...
- [已完成方案](../../plans/completed/btw.md)
- [实现调研](./implementation-research.md)

本文件回答一个具体问题：哪些代码可以直接沿用，哪些只能 copy 后修改，哪些只应作为设计参考。结论以当前 Pi `0.80.10`、仓库 strict TypeScript、`exactOptionalPropertyTypes`、`DESIGN.md` 和 BTW 首版边界为准。

## 1. 总结结论

### 优先直接复用本仓库代码

以下代码已经由 ext-core 或 concrete extension 提供，无需复制：

- `registerExtensionLifecycle` 和 `DisposerRegistry`：session runtime 创建、逆序 cleanup、重复 shutdown 保护。
...
- rpiv overlay 的 viewport clamp 和 bottom-anchor 算法：移除其主题/边框/固定布局，接入 `pi-btw` text primitives。
...
| --- | --- | --- | --- |
| `ExtensionLifecycleContext` | `packages/pi-ext-core/src/lifecycle.ts` | 直接由 extension lifecycle 使用 | BTW 不自己维护 session_start/shutdown 主循环 |
| `registerExtensionLifecycle` / `resources` | `packages/pi-ext-core/src/lifecycle.ts` | `registerExtensionLifecycle(pi, { key, start })` | cleanup 仍需 BTW 自身幂等 |
| `ExtensionLifecycleContext` | `packages/pi-ext-core/src/lifecycle.ts` | 作为 feature 的 runtime 输入 | 不把 `ctx` 和 runtime 写入模块级共享状态 |
| `sameSession` 语义 | `packages/pi-goal/src/feature.ts` | 在 BTW 内实现一个窄 guard | 不复制 Goal reducer/persistence |

实现最终落在 `packages/pi-btw/src/extension.ts`，通过 ext-core lifecycle 注册 cleanup；不把 runtime 放入 package-global state。

### 2.2 Custom UI 和 exactly-once settle

| Symbol/pattern | Source | 复用方式 |
| --- | --- | --- |
| `requestAsk()` 的 custom UI try/finally | `packages/pi-ask/src/feature.ts` | copy-adapt 到 `btw/feature.ts` |
| `AbortController` signal forwarding | `packages/pi-ask/src/feature.ts` | 直接采用独立 BTW controller，不能复用主 agent signal |
| `settle()` guard | `packages/pi-ask/src/component.ts` | copy-adapt 到 `btw/component.ts` |
| `dispose()` 的未 settle/已 settle 分支 | `packages/pi-ask/src/component.ts` | 直接采用结构，替换 Ask state 为 BTW state |
...
| Symbol | Source | 复用方式 |
| --- | --- | --- |
| `visibleWidth` | `packages/pi-btw/src/ui.ts` | 直接 import |
| `truncateToWidth` | `packages/pi-btw/src/ui.ts` | 直接 import |
| `wrap` | `packages/pi-btw/src/ui.ts` | 直接 import |
| `padToWidth` | `packages/pi-btw/src/ui.ts` | 直接 import |
| `keyGlyph` | `packages/pi-btw/src/ui.ts` | 直接 import |
| `formatKeymap` | `packages/pi-btw/src/ui.ts` | 直接 import |
| `renderScrollbar` | `packages/pi-btw/src/ui.ts` | 直接 import，BTW 自己管理 viewport |
...
| Pattern | Source | 复用方式 |
| --- | --- | --- |
| `modelRegistry.find()` + `hasConfiguredAuth()` | `packages/pi-auto-title/src/module.ts` | 借鉴查找/失败提示；BTW 使用当前 `ctx.model`，不解析 model ref |
| revision + active operation + dispose | `packages/pi-auto-title/src/module.ts` | copy-adapt 到 `sessionId + runtimeRevision + contextRevision + historyGeneration + requestRevision` |
| tree/compact/switch/fork abort guard | `packages/pi-goal/src/feature.ts` | copy-adapt 事件注册和 abort 顺序 |
...
| --- | --- | --- |
| extension integration harness | `packages/pi-btw/test/feature.test.ts` | 扩展或提取 BTW 专用 fixture |
| deferred async prompt | `packages/pi-auto-title/test/modules/auto-title.test.ts` | copy-adapt provider deferred response |
| repeated trigger/dispose abort tests | `packages/pi-auto-title/test/modules/auto-title.test.ts` | 改成 BTW request/overlay/shutdown tests |
| lifecycle idempotence | `packages/pi-btw/test/feature.test.ts` | 增加 BTW shutdown/reload/tree/compact cases |
...
- `Ctrl+C` close、旧的 reserved app rows、旧的 border/color layout：与当前 Pi BTW DESIGN 不一致。
...
- 完整 `BtwOverlayController`：其 visual styling 需要按当前 Pi BTW 语义重写。
...
2. 建议在 `packages/pi-btw` 或现有 LICENSE/第三方声明中列出实际采用的来源。
3. 采用窄算法时，在目标文件顶部加简短来源注释，例如“Adapted from ... under MIT License”，并说明已修改。
4. 不要把整份上游文件原样覆盖到 `pi-btw`，这样既会带入不需要的行为，也会让 attribution 和后续维护边界不清楚。
...
完成 `unknown` narrowing、readonly 状态、context budget、stop reason 和 generation guard 后，才放入 `packages/pi-btw/src/model.ts`。
...
命令、lifecycle、tree/compact abort、session cleanup 和主入口全部按 ext-core 与 `pi-btw` pattern 新写。不要从任一参考仓库复制入口文件。

## 9. 实施前最终判断

| 代码区域 | 决策 |
| --- | --- |
| `pi-btw` runtime/UI/test primitives | 直接复用 |
...
因此，实施不会从某个参考仓库整体复制到正式目录，而是先复用本地基础设施，再将约 5 个纯逻辑块作为 MIT attribution 下的改编起点。这样可以减少消息构造、响应归一化和 viewport 计算的工作量，同时保持 `pi-btw` 与 ext-core 的 lifecycle、UI 和安全边界不被上游实现带偏。
