...
- dry-run、fuzzy attempt、staging apply、baseline revalidation 与 commit 全都服从既有 `AbortSignal`/bridge cancellation。commit 前创建 request 级 rollback journal；journal 同时保存原始 mode 与 descriptor-read file identity。request 开始时打开并持有 workspace root directory fd，直到 result 或 rollback 完成；所有 source、journal、baseline、commit 与 rollback 的 parent traversal 都从此 fd 派生，因此替换同名 workspace pathname 不会把 request 导向另一棵目录树。commit/rollback 通过逐级 `O_DIRECTORY|O_NOFOLLOW` 固定 parent directory，workspace target reads 使用 `O_NOFOLLOW` file handles，temporary files 使用 `O_CREAT|O_EXCL|O_NOFOLLOW` file handles，避免中间或预置 temporary symlink 把读写导向 workspace 外；当前实现要求 Linux `/proc/self/fd` descriptor-relative 支持，非 Linux 在 parse/staging 前明确拒绝并要求用户显式选择 `Edit Mode: native`，不会静默路由。每个 path 在 replacement 前重新验证 content、mode、inode 与 metadata identity，并在 pathname install 后从 fixed parent重新验证 installed identity；rollback 只在目标仍匹配本 request 最近一次已知 commit identity 时恢复，发现外部改写则报告 `workspace state indeterminate`，不覆盖外部内容。request 创建的 parent directories也记录 inode，rollback 后仅删除仍是同一 inode且为空的目录。Node pathname `rename` 前的最后 identity check 仍不是任意外部 writer 的原子 compare-and-swap；同机不可信 writer也可在 exclusive temporary file创建后替换其目录项，post-install identity mismatch会进入 indeterminate recovery，但不等同于原子防护。failed install也不会通过竞态 pathname unlink清理 temporary entry；若该 entry仍存在，会保留它并报告 `workspace state indeterminate` 供显式检查。调用方不得并发修改同一目标，检测到的竞争会拒绝或进入 indeterminate recovery。取消或 commit error 后必须恢复本 request 已替换/删除的 path。rollback 无法完成时，工具返回 `workspace state indeterminate` 与 affected paths，模型必须先 `read` 后再行动。普通 socket 断线仅移除 subscriber，request 保留以允许同 id reconnect；用户 abort 发送显式 cancel request。cancel acknowledgement 若确认 rollback 完成才返回 cancellation；若 cancel 丢失或 mutation 已先 commit，返回 `workspace outcome is unknown`，不能把成功 mutation 当作普通 abort。parsing、queued 和 running request 都在不会继续 mutation 的第一个安全点完成取消，running request 等 executor rollback 后才发布最终 cancellation error。取消不产生 partial diagnostics 或 speculative snapshots。
...
- 同 path jobs 保持 coordinator serialization；无交集 path jobs 保持现有 worker concurrency。`maxQueueDepth` 是所有尚未 terminal 的 admitted request 的总上限，包含 parser、pending 与 running 状态；超过上限的 request 在保存 patch 或开始 parse 前被拒绝。request id 是 coordinator-memory-lifetime 内的 mutation id：同 id 重连订阅已存在 job 或读取短时缓存 result，不创建第二个 job。完成 result 最多保留 10 秒，且 completed replay cache同时限制为 128 records 与 8 MiB serialized compact responses；coordinator只维护一个指向最旧 retained record的 expiry timer。任一预算淘汰都会立即释放 record并保留有界 fingerprint tombstone。只要 id 仍在 record 或 tombstone 中，同 id 只会要求 `read`，绝不重新 mutation。coordinator restart 或 tombstone eviction 后，调用方必须使用新的 request id 并按普通 mutation 前 read/validation 语义处理。coordinator socket protocol revision 5 隔离旧的 fire-and-forget cancel、pathname-root和无 completed-cache预算的 server；每个 connection 可以承载多个 newline-delimited requests，response 按 request id 写回；Pi client 在自身 final response 后主动关闭。所有 terminal protocol response 都重新安排 idle shutdown；socket frame parser 会 drain 同一 chunk 的每个完整 request。SIGTERM/SIGINT 先停止 admission，通过同一 cancellation state machine取消 parsing、pending 与 running records，等待 running executor rollback及 terminal response flush，最后才关闭 sockets、server、lock与socket文件。
