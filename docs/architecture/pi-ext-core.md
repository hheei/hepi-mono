...
下沉到 core。鼠标与局部文本选择是明确记录的受限例外；它只提供 feature-neutral 的 terminal input、
region dispatch 与 local selection contract，不提供页面内容、clipboard policy 或新的 TUI layout tree。
