...
- 只使用公开 HTTP，统一位于 `/agentmemory/` 下：`health`、`session/start`、`observe`、`search`、
...
- 自动 recall admission 依照 `docs/mctx/spec.md` 由 `pi-mctx` 的 additive projection epoch / recall ledger 管理；Context Projection 在 Window 完成后原子发布 provider-visible splice，成功结果才可用于 LKG。
...
- session activation 恢复已到期 outbox；单个定时器按最早 retry deadline 唤醒有界 drain，不并行启动重复 worker。shutdown 取消定时器并执行有界清理。
- Pi `tool_result` 先于对应消息持久化；当前 tainted turn 的 memory-tool entry provenance 在 `agent_end` 读取已落库 branch 时补齐，不把工具输出提升为独立证据。
- `/ctx-status` 只读取 runtime 已观察状态和本地 outbox 计数，不为刷新状态发网络请求；`/agentmemory-health` 是唯一显式 fresh probe，bridge 关闭时直接报告 disabled。
...
## Automatic recall presentation

Context Projection remains provider-visible and session-transient; it never writes
recall text into Pi JSONL. After a successful publish, only interactive TUI mode
uses the ext-core-managed `aboveEditor` widget. The local ledger claims each event
with a lease and writes a presentation receipt only from the widget render that
returns the event, so concurrent presenters cannot duplicate an event and failed mounts can retry.
Headless/RPC mode has no widget. Status surfaces read at most three recent admitted
sources after removing control characters and collapsing whitespace; status refresh
does not probe the bridge.
## 不做
