...
`EmbeddingService` 是经用户明确授权的 single-consumer exception。`pi-embeddings` 需要先发布一个
...
执行 batch/backfill，也不决定 lexical fallback。provider lifecycle 属于 `pi-embeddings`；consumer 在 service
