...
本节取代上文「并发写入既不检测也不阻止」和「Publish 前不对帐」的决定，其余逐 path Publish、无 rollback 与取消语义不变。根据 [语义与安全改进计划](../plans/apply-patch-semantics-safety.md)，apply_patch 在发布前复核已读取的源基线；检测到外部修改时拒绝覆盖，Move 在删除源前再次复核。此方案增加读 I/O，换取对准备期间外部改写的可见拒绝，不引入 journal 或 coordinator。

复核和 rename/remove 之间仍有竞态窗口；这不是文件系统 compare-and-swap，也不阻止任意外部 writer。当前合同见 [apply_patch 架构](../architecture/apply-patch.md#验证与并发)。
