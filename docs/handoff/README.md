...
context-management command，也不是自动 compaction 或 session-history rewrite。
...
payload source；Handoff 不读取 extension-private state 或 retained source。

## 非目标

不包含 inheritance、`ctx_reduce` materialization、historian execution、fork graph projection、background
compaction、progress UI、wait/poll command 或旧 wrapper compatibility bridge。
