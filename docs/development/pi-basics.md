...
| Loadout bridge | `pi-ext-core` | Loadout publishes disabled skill keys; independent skill extensions read skill state | State is scoped to the shared Pi event bus. Producers register during session start and unregister during cleanup. Disabled skill keys reset during Loadout cleanup. | Unknown tool disable requests are no-ops. Skills are enabled unless Loadout explicitly disables their key. |
