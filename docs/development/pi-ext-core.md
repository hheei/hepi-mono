...
settings transport 与 mouse/local selection contract 是记录在 ADR 中的 bounded exceptions；第二个 consumer 出现前，
不得在它们上继续抽取 generic policy、content model、worker framework 或 shared dependency。
