...
`pi-basics` 的 `HePiModule` registry 只负责 `/hepi <subcommand>` 路由，不能自动提供独立 `/advisor`；现有 `/plan` 也是直接注册独立 command，因此 Advisor 应沿用这一方式。[模块 API](../../../packages/pi-basics/src/api/modules.ts) [命令路由](../../../packages/pi-basics/src/command/hepi-command.ts) [Plan feature](../../../packages/pi-basics/src/modules/plan/feature.ts)
...
Settings 公共类型和生命周期见 [settings API](../../../packages/pi-basics/src/api/settings.ts)。自动标题已经提供了 `provider/model` 选项、解析、认证验证和写入 `.pi/settings.json` 的可复用模式。[Auto Title settings](../../../packages/pi-basics/src/modules/auto-title/index.ts)
...
一次调用会读取 Pi 已解析的当前 branch context，删除正在执行但尚无 tool result 的 `advisor()` call，必要时补一个 user tail，然后调用指定 reviewer。Reviewer 请求显式使用 `tools: []`，返回 `plan`、`correction` 或 `stop signal` 类型的文本指导。[入口](../../../references/rpiv-mono/packages/rpiv-advisor/index.ts) [执行](../../../references/rpiv-mono/packages/rpiv-advisor/advisor/execute.ts) [上下文修正](../../../references/rpiv-mono/packages/rpiv-advisor/advisor/context.ts) [系统提示词](../../../references/rpiv-mono/packages/rpiv-advisor/prompts/advisor-system.txt)
...
BPX 把一次咨询扩展为 `solo`、`gut-check`、`council` 和 `debate`。它还包含 persona roster、stance、confidence/agreement、CLI backend、短语触发、when-stuck/on-done 自动触发、feedback mode、每 turn 调用预算和大型配置 UI。[入口](../../../references/bpx-mono/packages/bpx-consult/index.ts) [配置](../../../references/bpx-mono/packages/bpx-consult/src/config.ts) [Council](../../../references/bpx-mono/packages/bpx-consult/src/council.ts) [Debate](../../../references/bpx-mono/packages/bpx-consult/src/debate.ts)
...
见 [Context engine](../../../references/bpx-mono/packages/bpx-consult/src/context-engine.ts) 和 [Solo retry](../../../references/bpx-mono/packages/bpx-consult/src/solo.ts)。
...
它不是 executor，不能 edit、write、bash 或改变 session。Advisor 的普通模型文本不展示给用户；只有 `advise` tool 产生的结构化 note 才能进入主会话。[README](../../../references/pi-omplike-advisor/README.md) [Agent runtime](../../../references/pi-omplike-advisor/extensions/advisor.ts)
...
Plan 的 versioned boundary 和严格 decode 可作为模式参考。[Plan persistence](../../../packages/pi-basics/src/modules/plan/persistence.ts)
...
BPX 的 context engine 对 typed context 的这些 invariant 有系统测试，值得参考但不需要整体移植。[Context engine tests](../../../references/bpx-mono/packages/bpx-consult/tests/context-engine.test.ts)
...
```text
packages/pi-basics/src/modules/advisor/
  index.ts          exports
  feature.ts        session lifecycle、hooks、queue/drain
  command.ts        /advisor on|off|status
  settings.ts       provider/model/thinking storage + validation
  persistence.ts    AdvisorBoundary encode/decode/replay
  context.ts        bootstrap、turn delta、budget、tool pairing
  runtime.ts        long-lived Agent、private tools、reset/dispose
  feedback.ts       advice queue、dedup、custom message delivery
  prompt.ts         fixed system prompt
  model.ts          state、types、model ref、thinking resolution
```

是否进一步合并文件应由实现规模决定；不要预先建立通用“subagent framework”。Advisor 是当前唯一需要 persistent shadow Agent 的模块，过早共享会把 feature-specific lifecycle 泄漏到公共 API。
...
当前入口和 lifecycle controller 见 [Pi Basics entry](../../../packages/pi-basics/src/index.ts) [Lifecycle](../../../packages/pi-basics/src/runtime/lifecycle.ts) [Runtime context](../../../packages/pi-basics/src/runtime/context.ts)。

### 10.3 Settings 组合

Advisor 应创建唯一 id 的模块 provider，例如 `pi-basics-advisor`，其中 group id 为 `advisor`。入口把它与 auto-title、RTK 等 provider 一起传给 `combineSettingsProviders()`；不要直接注册另一个 id 为 `pi-basics` 或 `pi-basics-settings` 的 provider。Advisor storage 自己只读写 `.pi/settings.json` 的 `pi-basics.advisor` section，combined provider 只负责统一展示和分发 storage 生命周期。[Settings registry](../../../packages/pi-basics/src/api/settings.ts) [Combined settings](../../../packages/pi-basics/src/modules/setting/combined.ts)
...
Settings UI 遵循 `DESIGN.md` 和现有 enum 交互，不为 Advisor 新建模型 picker。[DESIGN](../../../DESIGN.md) [Settings component](../../../packages/pi-basics/src/modules/setting/component.ts)
...
`pi-basics` 的 auto-title 已经通过 event bus 使用外部 `@tintinweb/pi-subagents` RPC v2，因此复用它看起来比新增 Agent runtime 更轻。[Auto Title RPC client](../../../packages/pi-basics/src/modules/auto-title/index.ts) 但现有 RPC 的公开实现只提供：
