...
状态：已实现（待真实 TUI smoke test）

关联研究：[`pi-basics-btw-research.md`](./pi-basics-btw-research.md)

实施前复用清单：[`pi-basics-btw-reuse-inventory.md`](./pi-basics-btw-reuse-inventory.md)

本文件定义 `pi-basics` 首版 `/btw` 的功能边界、架构、预期行为、测试契约和实施顺序。实现位于 `packages/pi-basics/src/modules/btw`，本文保留为行为与验收契约。

## 1. 一句话方案

在 `@hheei/pi-basics` 内增加一个 session-scoped 的 `/btw <question>` 命令：复制当前 Pi 分支的对话上下文，用当前主模型发起一次无工具的独立侧问请求，在临时 TUI overlay 中显示答案；成功的侧问答可在同一个 Pi session 的后续 `/btw` 请求中继续使用，但永远不写入主 agent transcript、磁盘或主 agent 的工具上下文。
...
- `@hheei/pi-basics` 必须在 `peerDependencies` 中声明 `@earendil-works/pi-ai >=0.80.10`，因为生产代码会直接 import `@earendil-works/pi-ai/compat`。
...
```text
packages/pi-basics/src/modules/btw/
...
packages/pi-basics/test/modules/btw/
  model.test.ts
  executor.test.ts
  feature.test.ts
  component.test.ts
...
入口在 `packages/pi-basics/src/index.ts` 创建 `createBtwFeature()`，并在 `piBasicsExtension()` 注册其 command/lifecycle hooks。`BtwFeature` 持有当前 `HePiRuntimeContext` 和 session-local state，不把 runtime 放在模块全局。
...
1. 更新 `packages/pi-basics/README.md`。
2. 添加 package exports（只暴露确实需要的 feature/model 类型）。
3. 更新 `packages/pi-basics/package.json` peer dependency 和 ship manifest 覆盖。
...
- 现有 `pi-basics` tests、typecheck、check 不回归。
...
```bash
bun test packages/pi-basics/test/modules/btw
bun run typecheck
bun run check
bun test
```
