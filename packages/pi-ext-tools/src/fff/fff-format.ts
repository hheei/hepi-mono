import { type TruncationResult, truncateHead, truncateLine } from "@earendil-works/pi-coding-agent";
import {
...
function pickSuggestedReadPath(items: GrepMatch[]): {
	...
	return {
		reason,
		...(suggested?.relativePath === undefined ? {} : { path: suggested.relativePath }),
		...(definition?.relativePath === undefined ? {} : { expandedPath: definition.relativePath }),
	};
}
...
export function buildGrepText(
	...
	return {
		...
		...(matchLimitReached === undefined ? {} : { matchLimitReached }),
		...(finalized.truncation === undefined ? {} : { truncation: finalized.truncation }),
