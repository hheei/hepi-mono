...
import { suspendWidgets } from "@hheei/pi-ext-core";
import { describe, expect, test } from "vitest";
import type { TaskState } from "../../src/todo/model.js";
import { createTodoWidget as createManagedTodoWidget } from "../../src/todo/widget.js";
...
describe("todo widget", () => {
	...
	test("registers once and requests render on refresh", () => {
		...
		const widget = createTodoWidget(h, state([task(1, "x", "pending")]))!;
		component(h);
		expect(h.calls[0]?.key).toBe("pi-ext-tools:todo");
	...
	test("is suspended and restored through the core Settings lease", () => {
		...
		const lease = suspendWidgets(h.pi);
		expect(h.calls.at(-1)?.content).toBeUndefined();
		lease.release();
	...
	test("orders in-progress, pending, then temporary blocked tasks", () => {
		...
		expect(lines.slice(1).flatMap((line: string) => line.match(/#\d+/)?.[0] ?? [])).toEqual([
			"#3",
			"#1",
			"#4",
			"#2",
