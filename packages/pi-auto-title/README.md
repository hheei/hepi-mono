...
Enable automatic titles and select a model in `/ext-settings`. Use `/auto-title` to generate or replace a title manually. Automatic generation runs only on startup and new sessions. Settings persist under `pi-basics.auto-title` in global `~/.pi/agent/settings.json`.
