...
Enable automatic titles and select a model in `/ext-settings`. Title generation always uses
thinking level `off`. Use `/auto-title` to generate or replace a title manually. Automatic
generation runs only on startup and new sessions. Settings persist under
`pi-basics.auto-title` in `<cwd>/.pi/settings.json`.
