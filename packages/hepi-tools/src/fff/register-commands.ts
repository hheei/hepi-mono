				...
				return {
					render(width: number) {
						...
						for (let i = 0; i < FEATURE_DEFINITIONS.length; i += 1) {
							...
							add(
								selected
									? `${prefix}${theme.fg("accent", label)}`
									: `${prefix}${theme.fg("text", label)}`,
							);
						...
						lines.push("");
						add(
							theme.fg("dim", `Enabled: ${Array.from(draft).length}/${FEATURE_DEFINITIONS.length}`),
						);
						cachedLines = lines;
						return lines;
					},
		...
		handler: async (_args, ctx) => {
			...
			const report = buildStatusReport({
				...
				features: FEATURE_DEFINITIONS.map((feature) => ({
					label: feature.label,
					enabled: deps.isFeatureEnabled(feature.id),
				})),
			});
