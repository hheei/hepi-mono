...
const ANCHORED_READ_INFORMATIONAL_LINE_PATTERN =
	/^\s*(?:$|<\/?file>|\.{3}|\[[^\]]+\]|Read\s+.+:\s+\d+\s+lines\b)/;
...
function looksLikeAnchoredLineOutput(
	text: string,
	parseLineNumber: (line: string) => number | undefined,
): boolean {
	let matchCount = 0;
...
function truncateAnchorSafeReadLines(
...
function compactAnchoredReadText(
	...
	if (
		...
		renderAnchorSafeReadText(parts, lines).length > compaction.truncate.maxChars
	) {
...
function compactBashText(
	...
	applyConditionalTechnique(
		state,
		compaction.filterBuildOutput,
		(t) => filterBuildOutput(t, command),
		"build",
	);
	applyConditionalTechnique(
		state,
		compaction.aggregateTestOutput,
		(t) => aggregateTestOutput(t, command),
		"test",
	);
	applyConditionalTechnique(
		state,
		compaction.compactGitOutput,
		(t) => compactGitOutput(t, command),
		"git",
	);
	applyConditionalTechnique(
		state,
