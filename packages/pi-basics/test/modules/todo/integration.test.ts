...
import { createLoadoutInventory } from "../../../src/modules/loadout/inventory.js";
import {
	createTodoFeature,
	TODO_PARAMETERS,
...
import type { TodoSnapshot } from "../../../src/modules/todo/state.js";
...
describe("Todo integration", () => {
	...
	test("exposes todo to Loadout inventory", () => {
		const host = harness();
		createTodoFeature(host.pi);
			...
			getAllTools: () =>
				host.tools.map((tool) => ({
					...
					sourceInfo: {
						...
						source: "pi-basics",
						path: "packages/pi-basics/src/modules/todo/index.ts",
						origin: "package" as const,
					},
				})),
			getCommands: () => [],
			getToolDefinition: (name) =>
				name === TODO_TOOL_NAME ? { promptSnippet: TODO_PROMPT_SNIPPET } : undefined,
		});
		const item = inventory.items.find(({ name }) => name === TODO_TOOL_NAME);
		expect(item?.conflictGroup).toBe("tool:todo");
		expect(item?.description).toBe(TODO_PROMPT_SNIPPET);
	});
});
