...
import { createLoadoutStorage } from "../../../src/modules/loadout/storage.js";
...
describe("loadout storage", () => {
	...
	test("rejects malformed JSON, non-object sections, and non-boolean entries", async () => {
		...
		try {
			...
			await writeFile(paths.globalPath, JSON.stringify({ "pi-loadout": [] }));
			await expect(storage.load()).rejects.toThrow("object");
			await writeFile(paths.globalPath, JSON.stringify({ "pi-loadout": { "tool:read": "yes" } }));
