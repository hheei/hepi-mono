...
function requiredLine(line: string | undefined): string {
	if (line === undefined) throw new Error("expected rendered line");
	return line;
}
...
describe("settings renderer", () => {
	test("renders wide description and narrow fixed Value area within width", async () => {
		...
		expect(wideLines.some((line) => line.includes("╰") && line.endsWith("╯"))).toBe(true);
		...
		expect(plain(renderSettings({ controller: wrappedController, theme, width: 75 }))).toContain(
			"...",
		);
	});
	...
	test("dims Description panel except edit draft Value and cursor", async () => {
		...
		expect(navigationOutput).toContain(dim("name"));
		...
		expect(requiredLine(navigationValueRow)).not.toContain(ansi.accent);
		...
		expect(editingOutput).toContain(dim("name"));
		...
		expect(requiredLine(editingValueRow)).not.toContain(dim("draft-value█"));
	});
	...
	test("keeps wide Description rows and footer stable across content and edit state", async () => {
		...
		expect(editing.findIndex((line) => stripAnsi(line).includes("↵ save"))).toBe(
			3 + layout.descriptionHeight,
		);
		...
		expect(panelContent(longDetail[1]!)).toBe("long-id");
		expect(panelContent(longDetail[2]!)).toBe("");
		expect(panelContent(longDetail[3]!)).not.toBe("");
		expect(panelContent(longDetail[4]!)).toEndWith("...");
		expect(panelContent(longDetail[5]!)).toBe("");
		expect(panelContent(longDetail[6]!)).toBe("Origin: @pi-basics");
		expect(panelContent(longDetail[7]!)).toBe("Value: long-id-value");
	...
	test("renders main tabs with structural active opening, white inactive tab, and closed inactive tab", async () => {
		...
		expect(requiredLine(stripAnsi(requiredLine(settingsTabs[1])))).toBe(
			stripAnsi(requiredLine(loadoutTabs[1])),
		);
		expect(loadoutLines.map(stripAnsi)).toContain("↕ navigate · ↔ switch · ␣ edit · ⎋ close");
	...
	test("renders exact state footers and drops lowest-priority hints first", async () => {
		...
		expect(footerAt(48)).toBe("↕ navigate · ↔ switch · ␣ edit · ⎋ close");
		expect(footerAt(47)).toBe("↕ navigate · ↔ switch · ␣ edit · ⎋ close");
		expect(footerAt(37)).toBe("↕ navigate · ↔ switch · ␣ edit");
		expect(footerAt(21)).toBe("↕ navigate · ↔ switch");

		controller.beginEdit();
		expect(footerAt(20)).toBe(editFooter);
		expect(footerAt(19)).toBe("↵ save · ⎋ cancel");
	});
	...
	test("keeps committed list value while draft appears only in editor", async () => {
		...
		expect(narrow).toContain("↵ save · ⎋ cancel");
	});
