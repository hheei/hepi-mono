...
export class SettingsController {
	...
	async close(): Promise<void> {
		...
		const providers = this.state.providers.map(({ provider }) => ({
			provider,
			state: cloneState(this.state.committed[provider.id] ?? {}),
			previous: cloneState(this.#committed.get(provider.id) ?? {}),
		}));
		const failures: Array<{ readonly providerId: string; readonly error: unknown }> = [];
		for (const { provider, state, previous } of providers) {
			const changes = changedValues(previous, state);
			if (!this.#dirty.has(provider.id) || changes.length === 0) continue;
			try {
