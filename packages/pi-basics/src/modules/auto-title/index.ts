...
const autoTitleFields: readonly HePiSettingField[] = [
	{
		id: AUTO_TITLE_FIELD,
	...
	},
	{
		...
		type: "text",
		defaultValue: "",
		description: "Exact provider/model used for title generation.",
	...
	const storage = {
		...
		save: async (state: HePiSettingsState, ctx: HePiContext) => {
			...
			await options.onPersisted?.(
				values[AUTO_TITLE_FIELD] === true && typeof model === "string" ? model : undefined,
			);
		},
	};
	return {
		...
		title: "Pi Basics",
		origin: "@pi-basics",
		groups: [{ id: AUTO_TITLE_GROUP, title: "Automatic titles", fields: autoTitleFields }],
		storage,
		onLoad: async (state, ctx) => {
			...
			if (typeof model !== "string" || !model)
				throw new Error("Title model is required when automatic titles are enabled");
			parseModelRef(model);
			await options.validate?.(model, ctx);
			await options.onPersisted?.(model);
		},
		onChange: async (change, ctx) => {
			...
			if (!enabled) return;
			if (!model) throw new Error("Title model is required when automatic titles are enabled");
			parseModelRef(model);
			await options.validate?.(model, ctx);
			if (change.fieldId === AUTO_TITLE_FIELD && change.value === true)
				await options.prepareEnable?.(model);
		},
	};
}
