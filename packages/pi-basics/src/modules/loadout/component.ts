...
type LoadoutHostTheme = Pick<Theme, "fg" | "bold">;

function renderForeground(theme: LoadoutHostTheme, color: string, text: string): string {
	switch (color) {
		case "accent":
		...
		default:
			throw new Error(`Unsupported loadout theme color: ${color}`);
	}
}
...
export interface LoadoutComponentOptions {
	...
	readonly theme: LoadoutHostTheme;
	...
	return {
		render(nextWidth: number): string[] {
			return renderLoadout({
				...
				theme: {
					fg: (color, text) => renderForeground(options.theme, color, text),
					bold: options.theme.bold,
				},
				width: nextWidth,
