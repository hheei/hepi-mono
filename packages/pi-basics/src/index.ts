	...
	const lifecycle = new HePiLifecycleController({
		onStart: async (runtime) => {
			...
			const autoTitleProvider = createAutoTitleProvider(runtime, (model) => {
				...
				autoTitleCoordinator = model ? createAutoTitleCoordinator(runtime, model) : undefined;
			});
			const settingsContext = {
				...runtime.ctx,
				sessionId: runtime.ctx.sessionManager.getSessionId(),
			};
			void Promise.resolve(autoTitleProvider.storage.load(settingsContext))
				.then((state) => autoTitleProvider.onLoad?.(state ?? {}, settingsContext))
				.catch((error: unknown) =>
