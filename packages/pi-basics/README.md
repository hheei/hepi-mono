...
Loadout stores global choices in `~/.pi/agent/setting.json` and project choices in `<cwd>/.pi/setting.json`, under `pi-basics-loadout`. It opens on project scope when `<cwd>/.pi/` exists, otherwise global scope. Within each kind, built-in/core items appear first; remaining items are ordered by package/source, then by name inside that package. Persisted entries use stable `kind:name` identities so extension source paths may change across reloads without losing the choice. Existing source-scoped entries remain readable and are migrated when that item is next changed.
