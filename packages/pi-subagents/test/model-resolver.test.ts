...
describe("resolveModel", () => {
	...
	describe("getAvailable filtering", () => {
		it("uses getAvailable when present (filters to configured models)", () => {
			const available = [MODELS[0], MODELS[2]].filter((model) => model !== undefined); // only opus and haiku
		...
		it("exact match fails when model is not in getAvailable (no auth)", () => {
			const available = [MODELS[0]].filter((model) => model !== undefined); // only opus available
		...
		it("fuzzy matches against available models only", () => {
			const available = [MODELS[2]].filter((model) => model !== undefined); // only haiku available
