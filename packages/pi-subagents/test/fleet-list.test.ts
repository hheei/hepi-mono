...
/** A no-op core handle so a record is "openable" by default. */
const FAKE_SESSION = {
...
describe("FleetList rendering", () => {
	...
	it("hides agents that have no session yet (pending)", () => {
		const agents = [
			...
			makeRecord({ id: "pending", description: "queued one", status: "queued", handle: undefined }),
		];
		const lines = harness(agents).render();
...
describe("FleetList overlay lifecycle", () => {
	...
	it("keeps the cursor on the viewed agent after closing, even if the list reordered", async () => {
		...
		const agents = [
			makeRecord({ id: "a1", description: "one", handle: fakeSession as any }),
			makeRecord({ id: "a2", description: "two", handle: fakeSession as any }),
			makeRecord({ id: "a3", description: "three", handle: fakeSession as any }),
		];
		const h = harness(agents);
