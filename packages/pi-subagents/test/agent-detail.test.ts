...
import type { LoadoutResourceDetailContext } from "@hheei/pi-ext-core";
...
const BACKSPACE = "\x7f";
const ESCAPE = "\x1b";
const INPUT_CONTEXT: LoadoutResourceDetailContext = { openEditor: async () => undefined };

function sendInput(
	detail: AgentDetail,
	input: string,
	context: LoadoutResourceDetailContext = INPUT_CONTEXT,
) {
	return detail.handleInput(input, context);
}
...
function setup(
	...
	registry: Parameters<typeof createAgentDetail>[2] = AUTH_REGISTRY,
) {
...
describe("createAgentDetail", () => {
	it("clones a built-in agent into the global agents dir from Global scope", async () => {
		...
		await sendInput(detail, ENTER, { openEditor: async () => "Custom description x" });
		detail.flush?.();
	...
	it("cycles thinking with Tab inside the selector and confirms both on Enter", async () => {
		...
		await sendInput(detail, ENTER); // confirm
		detail.flush?.();
	...
	it("cycles thinking within off…max and never offers inherit", async () => {
		...
		// option in the cycle.
		for (let i = 0; i < 8; i++) await sendInput(detail, TAB);
		await sendInput(detail, ENTER);
		detail.flush?.();
	...
	it("selects a provider/model from the cycler and inherits on the leading option", async () => {
		...
		await sendInput(detail, ENTER); // reopen; current option selected
		await sendInput(detail, UP);
		await sendInput(detail, UP); // back to inherit
		await sendInput(detail, ENTER);
		detail.flush?.();
		expect(readContent(path)).not.toContain("model:");
	});
	...
	it("excludes models without configured auth from the cycler options", async () => {
		...
		await sendInput(detail, DOWN); // wraps back to inherit (modulo)
	...
	it("preserves a configured unauthenticated model as the current option", async () => {
		const { detail, path } = setup(
			...
			{
				getAvailable: () => [{ provider: "anthropic", id: "claude-haiku-4-5", name: "Haiku" }],
				hasConfiguredAuth: (model) => model.provider === "anthropic",
			},
		);
		for (let i = 0; i < 2; i++) await sendInput(detail, DOWN);
		...
		expect(detail.render(60).join("\n")).toMatch(/Model\s+anthropic\/claude-haiku-4-5/);
		await sendInput(detail, DOWN); // wraps back to inherit
		expect(detail.render(60).join("\n")).toMatch(/Model\s+inherit/);
		await sendInput(detail, DOWN); // → the preserved current value
		await sendInput(detail, ENTER);
		detail.flush?.();
		expect(readContent(path)).toContain('model: "cx/gpt-5.6-luna"');
	});
	...
	it("keeps an authenticated current model in alphabetical order", async () => {
		...
		expect(detail.render(60).join("\n")).toMatch(/Model\s+cx\/gpt-5\.6-luna/);
		await sendInput(detail, UP); // back to the current value
		await sendInput(detail, ENTER);
		detail.flush?.();
	...
	it("applies the accent theme to the focused row", async () => {
		const { detail } = setup();
		const theme: Theme = Object.assign(Object.create(Theme.prototype), {
	...
	it("opens native editor with body prefill and buffers submit or cancel", async () => {
		...
		for (let i = 0; i < 3; i++) await sendInput(cancelled.detail, DOWN);
		await sendInput(cancelled.detail, ENTER, { openEditor: async () => undefined });
		cancelled.detail.flush?.();
	...
	it("clones a submitted Body for a built-in agent", async () => {
		const { detail, root } = setupDefault();
		for (let i = 0; i < 3; i++) await sendInput(detail, DOWN);
		await sendInput(detail, ENTER, { openEditor: async () => "Cloned body." });
		detail.flush?.();
	...
	it("removes whole code points with backspace", async () => {
		const { detail, path } = setup();
		await sendInput(detail, "中");
		await sendInput(detail, "a");
		await sendInput(detail, BACKSPACE);
		await sendInput(detail, ENTER);
		detail.flush?.();
	...
	it("keeps a built-in agent's Identity read-only, accent while focused and dim otherwise", async () => {
		const { detail, root } = setupDefault();
		await sendInput(detail, "x"); // identity edit is discarded
		await sendInput(detail, ENTER);
		detail.flush?.();
	...
	it("buffers Global and Project edits separately and flushes each to its own file", async () => {
		...
		await sendInput(detail, "G");
		await sendInput(detail, ENTER);
		for (let i = 0; i < 3; i++) await sendInput(detail, DOWN);
		await sendInput(detail, ENTER, { openEditor: async () => "Global edited body." });
		// …then the same agent is edited from the Project scope.
		detail.onScopeChange?.("project");
		await sendInput(detail, ENTER, { openEditor: async () => "Project edited body." });
		for (let i = 0; i < 3; i++) await sendInput(detail, UP);
		await sendInput(detail, "P");
		detail.flush?.();
