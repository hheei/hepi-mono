...
export class ConversationViewer implements Component {
	...
	constructor(
		private tui: TUI,
		private session: ConversationSubagentHandle,
	...
	render(width: number): string[] {
		...
		if (tokens > 0) {
			const percent = this.record.contextPercent ?? null;
	...
	private buildContentLines(width: number): string[] {
		...
		const th = this.theme;
		const messages = this.session.transcript().entries;
		const lines: string[] = [];
		...
		for (const msg of messages) {
			if (msg.role === "user") {
				...
				for (const name of msg.toolCalls) {
