...
A [Pi](https://pi.dev) extension that provides Claude Code-style autonomous sub-agents. Child execution is coordinated through `@hheei/pi-ext-core`; this package owns agent profiles, tool policy, records, delivery, and the `agent` tools.
...
- **Parallel background agents** — spawn multiple agents through the shared core coordinator (default active-turn cap: 2) and receive completion notifications
...
- **Finite graceful turn limits** — `max_turns` is required and positive; core sends one wrap-up steer and allows a fixed five-turn grace before hard cancellation
...
- **Schedule subagents** — pass `schedule` to the `Agent` tool to fire on cron / interval / one-shot. Session-scoped jobs with PID-locked persistence; results land via the same `subagent-notification` followUp path as manual background completions; manage via `/subagent-schedules list` and `/subagent-schedules remove <job-id>`
...
```bash
pi install npm:@hheei/pi-subagents
```
...
Schedules are **session-scoped**: they reset on `/new` and restore on `/resume`. List via `/subagent-schedules list` and cancel via `/subagent-schedules remove <job-id>` (creation is the `Agent` tool's job — there is no parallel manual-create wizard). Storage at `<cwd>/.pi/subagent-schedules/<sessionId>.json` with PID-based file locking for cross-instance safety.
...
| `max_turns` | required | Positive max agentic turns. Core owns a fixed five-turn wrap-up grace; unlimited runs are not supported |
...
| `output_transcript` | `true` | Write this subagent's `.output` transcript. Set `false` to suppress it |
...
- Orca-managed extensions (`orca-*`) never bind to a subagent session. Orca identifies status-hook ownership by process ID, while subagents run as in-process Pi sessions; binding them would report a child's lifecycle and final result as parent-pane activity.
...
| `max_turns` | number | yes | Positive max agentic turns; core adds a fixed five-turn wrap-up grace |
...
Runtime tuning values exposed by the `pi-settings` provider persist in Pi's `settings.json` under the `pi-subagents` section. `/agents` owns agent administration; generic extension settings remain in `/ext-settings`.

- **Global:** `<getAgentDir()>/settings.json`, section `pi-subagents`.
- **Project:** Pi's project settings section, when supported by the active settings host.

**Precedence:** the core settings host merges global and project values. Missing fields fall back to finite max turns and the shared core coordinator budget (active-turn cap `2`). The grace period is always `5` turns and cannot be widened by extension settings.
...
**Output transcript** (`outputTranscript`, default `true`): the settings provider controls whether each subagent's bounded `.output` transcript is written. A custom agent's `output_transcript` frontmatter overrides this per agent.
...
**Failure behavior:** invalid settings are rejected per field by the core JSON storage; the extension keeps its finite defaults and reports persistence failures through the settings host.
