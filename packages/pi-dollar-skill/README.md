...
When `pi-loadout` is present, disabled skills are filtered through the `pi-basics` bridge. This package does not depend on `pi-loadout`. Do not load another extension that transforms the same `$skill` syntax.
