...
describe("Caveman settings", () => {
	test("registers once and disposes through the runtime registry", async () => {
		...
		const unregister = registerCavemanSettings(pi);
		expect(unregister).toBeFunction();
		expect(() => registerCavemanSettings(pi)).toThrow(
			"HePi settings provider id collision: pi-caveman",
		);
		expect(registry.get(CAVEMAN_SETTINGS_PROVIDER_ID)?.origin).toBe("@hheei/pi-caveman");
		unregister?.();
		expect(registry.get(CAVEMAN_SETTINGS_PROVIDER_ID)).toBeUndefined();
	});
