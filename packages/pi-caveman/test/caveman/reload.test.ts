...
import piCavemanExtension, { CAVEMAN_SETTINGS_PROVIDER_ID } from "../../src/index.js";
...
describe("Caveman lifecycle", () => {
	test("keeps reload handlers from registering settings twice", async () => {
		...
		try {
			piCavemanExtension(harness.pi, { settingsFilePath });
			await harness.emit("session_start");
			piCavemanExtension(harness.pi, { settingsFilePath });
			await harness.emit("session_start");
			expect(
				getHepiRuntimeSettingsRegistry(harness.pi).get(CAVEMAN_SETTINGS_PROVIDER_ID),
			).toBeDefined();
