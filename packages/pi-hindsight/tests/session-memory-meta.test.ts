...
describe("session memory metadata", () => {
	...
	it("preserves concurrent tag updates", async () => {
		const cwd = mkdtempSync(join(tmpdir(), "pi-hindsight-meta-"));

		await Promise.all([
			addSessionMemoryTag(cwd, "/tmp/session.jsonl", "domain:one"),
			addSessionMemoryTag(cwd, "/tmp/session.jsonl", "domain:two"),
		]);

		expect((await readSessionMemoryMeta(cwd, "/tmp/session.jsonl")).tags).toEqual(
			expect.arrayContaining(["domain:one", "domain:two"]),
		);
	});
});
