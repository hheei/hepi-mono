...
describe("retain receipts", () => {
	...
	it("preserves concurrent receipt appends", async () => {
		const cwd = mkdtempSync(join(tmpdir(), "pi-hindsight-receipts-"));
		const receipt = (documentId: string) => ({
			bankId: "project-bank",
			documentId,
			queueJobId: `job-${documentId}`,
		...
		await Promise.all([
			appendRetainReceipt(cwd, receipt("doc-one")),
			appendRetainReceipt(cwd, receipt("doc-two")),
		]);

		expect((await listRetainReceipts(cwd)).map((item) => item.documentId)).toEqual(
