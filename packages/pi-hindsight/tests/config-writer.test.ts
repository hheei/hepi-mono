...
describe("settings.json config writer", () => {
  it("writes project config under the pi-hindsight section", async () => {
    ...
    const result = await writeProjectConfig(cwd, buildProjectConfigPatch({ projectBankId: "bank" }));
  ...
  it("preserves sibling settings sections", async () => {
    ...
    writeFileSync(join(cwd, ".pi", "settings.json"), JSON.stringify({ "pi-ponytail": { x: true } }));
  ...
  it("writes global config into the user settings section", async () => {
    const home = mkdtempSync(join(tmpdir(), "pi-hindsight-global-"));
    await writeGlobalConfig(buildProjectConfigPatch({ baseUrl: "http://global" }), [], home);

    expect(readGlobalConfig(home)).toMatchObject({ hindsight: { baseUrl: "http://global" } });
    expect(JSON.parse(readFileSync(globalConfigPath(home), "utf8"))).toHaveProperty("pi-hindsight");
  });

  it("removes selected project overrides without deleting the settings section", async () => {
    const cwd = mkdtempSync(join(tmpdir(), "pi-hindsight-config-"));
    await writeProjectConfig(cwd, buildProjectConfigPatch({ projectBankId: "bank", recallBudget: "high" }));
