...
import {
	...
	readImportManifest,
} from "../extensions/imports/import-plan.js";
import {
...
describe("Pi session import", () => {
	...
	it.each(equivalentPathVariants)("accepts equivalent normalized project cwd paths: $name", async ({
		sessionCwd,
	}) => {
		const dir = mkdtempSync(join(tmpdir(), "pi-hindsight-import-"));
		mkdirSync(join(dir, ".git"));
	...
	it.each([
	...
	])("revalidates project cwd during project import execution after discovery: $name", async ({
		replacementSession,
		expectedError,
	}) => {
		const project = mkdtempSync(join(tmpdir(), "pi-hindsight-project-"));
		...
		try {
			const { importProjectSessions: importProjectSessionsWithSwappedRead } = await import(
				"../extensions/imports/import-sessions.js"
			);
