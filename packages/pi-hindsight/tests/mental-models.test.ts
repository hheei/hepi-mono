...
describe("mental model inject filter", () => {
	it("keeps bank-global and matching project models only", async () => {
		const { filterMentalModelsForInject } = await import(
			"../extensions/lifecycle/mental-models.js"
		);
		const models = [
...
describe("mental model injection and retain safety", () => {
	...
	it("does not retain injected mental model blocks", () => {
		...
		if (job === undefined) throw new Error("Expected retain job");
	...
	it("does not retain MCTX knowledge snapshot markers", () => {
		const messages = [
			{
				role: "custom",
				customType: "pi-injected-knowledge",
				content: "<hindsight-knowledge>derived context</hindsight-knowledge>",
				details: injectedKnowledgeMarker(["mental-model:architecture"]),
				timestamp: 1,
			},
			{ role: "user", content: "Keep source evidence.", timestamp: 2 },
		] as unknown as AgentEndEvent["messages"];
		const job = buildRetainJob({
			config: DEFAULT_CONFIG,
			cwd: "/repo",
		...
		if (job === undefined) throw new Error("Expected retain job");
		expect(job.item.content).toContain("Keep source evidence.");
		expect(job.item.content).not.toContain("derived context");
	});
