...
describe("retain queue", () => {
	...
	it.each(
		stressCases,
	)("quarantines malformed active lines and flushes valid queued jobs under stress %#", async (index) => {
		const path = join(mkdtempSync(join(tmpdir(), "pi-hindsight-q-")), "q.jsonl");
		writeFileSync(
			path,
	...
	it.each(
		stressCases,
	)("moves exhausted failed jobs to the dead-letter queue under stress %#", async (index) => {
	...
	it.each(
		stressCases,
	)("does not lose jobs when multiple processes enqueue concurrently under stress %#", async (run) => {
		const path = join(mkdtempSync(join(tmpdir(), "pi-hindsight-q-")), "q.jsonl");
		await Promise.all(
