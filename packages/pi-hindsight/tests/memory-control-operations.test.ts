...
describe("memory control operations", () => {
	...
	it("config patch rejects unknown keys and dry-runs by default", async () => {
		...
		const disk = JSON.parse(readFileSync(join(cwd, ".pi", "settings.json"), "utf8")) as {
			"pi-hindsight"?: {
				setupComplete?: boolean;
				banks?: { project?: { bankId?: string } };
			};
		};
		expect(disk["pi-hindsight"]?.setupComplete).toBe(true);
		expect(disk["pi-hindsight"]?.banks?.project?.bankId).toBe("kai-coding");
	});
