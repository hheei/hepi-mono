...
function writeHindsightConfig(cwd: string, config: Record<string, unknown>): void {
...
describe("extension hooks", () => {
	...
	it("skips bank ensure and recall when setup is incomplete", async () => {
		...
		// No settings config or runtime state means the setup gate remains closed.
	...
	it("retains global memory through configured global bank", async () => {
		...
		mkdirSync(join(cwd, ".git"));
		mkdirSync(join(cwd, ".pi"));
		writeHindsightConfig(cwd, { banks: { user: { enabled: true, bankId: "global-luxus" } } });
	...
	it("ensures global bank even when project bank is disabled", async () => {
		...
		mkdirSync(join(cwd, ".pi"));
		writeHindsightConfig(cwd, {
			banks: { project: { enabled: false }, user: { enabled: true, bankId: "global-bank" } },
