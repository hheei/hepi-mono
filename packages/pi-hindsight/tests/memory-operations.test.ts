...
describe("memory operations", () => {
	it("configures memory profiles without implicit project/global overrides", async () => {
		...
		let written = JSON.parse(readFileSync(join(cwd, ".pi", "settings.json"), "utf8")) as {
			"pi-hindsight": Record<string, unknown>;
		};
		let config = written["pi-hindsight"];
		expect(config.banks).toMatchObject({
			project: { enabled: false },
			user: { enabled: true, bankId: "shared" },
		});

		await operations.configure(cwd, { timeoutMs: 1234 });
		written = JSON.parse(readFileSync(join(cwd, ".pi", "settings.json"), "utf8")) as {
			"pi-hindsight": Record<string, unknown>;
		};
		config = written["pi-hindsight"];
		expect(config.banks).toMatchObject({
			project: { enabled: false },
			user: { enabled: true, bankId: "shared" },
		});
		expect(config.hindsight).toMatchObject({ timeoutMs: 1234 });
