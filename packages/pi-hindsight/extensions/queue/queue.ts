...
import { type JsonlQueueFileSummary, JsonlQueueStore } from "./jsonl-queue-store.js";
...
export { isQueueLockOwnerStale, type QueueLockOwner, RETAIN_QUEUE_LOCK } from "./queue-lock.js";
...
export async function flushRetainQueue(
	...
	return withQueueLock(path, async () => {
		...
		for (const [index, job] of jobs.entries()) {
			...
			try {
				const response = await deliverRetainJob(client, job, resolvedOptions.signal);
				...
				if (resolvedOptions.signal?.aborted) throw error;
