...
/** Compact overlay hub; detailed field editing remains outside this popup. */
export function createHindsightHubComponent(
	facts: ReadonlyArray<readonly [string, string]>,
	theme: ThemeLike,
	done: (action: SetupActionId | null) => void,
): Component {
	const hubFacts = facts.slice(0, 3);
	return {
		render(width: number): string[] {
			const innerWidth = Math.max(0, width - 2);
			const border = (left: string, fill: string, right: string): string =>
				theme.fg("borderMuted", `${left}${fill.repeat(Math.max(0, width - 2))}${right}`);
			const box = (content: string): string => {
				if (width < 2) return truncateToWidth(content, width);
				return `${theme.fg("borderMuted", "│")}${padVisibleRight(content, innerWidth)}${theme.fg("borderMuted", "│")}`;
			};
			const lines = [
				border("╭", "─", "╮"),
				box(` ${theme.fg("accent", theme.bold("Hindsight"))}`),
				box(""),
				...(hubFacts.length > 0
					? hubFacts.map(([label, value]) =>
							box(
								fitColumns(
					...
					: [box(theme.fg("muted", " Status unavailable"))]),
				box(""),
				box(theme.fg("dim", " g setup · m mode · x next-opt-out · f flush · o doctor")),
				box(theme.fg("dim", " d deployment · i import · t templates · n init · q close")),
				border("╰", "─", "╯"),
			];
		...
		handleInput(data: string): void {
			...
			if (!intent) return;
			const result = applySetupIntent([], { tabIndex: 0, selectedByTab: {} }, intent);
...
export function createSetupComponent(
	...
	return {
		render(width: number): string[] {
			Object.assign(state, normalizeSetupUiState(tabs, state));
			const tab = currentSetupTab(tabs, state);
