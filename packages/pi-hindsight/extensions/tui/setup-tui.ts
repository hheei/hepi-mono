...
import { createHindsightHubComponent, createSetupComponent } from "./setup-tui-render.js";
...
async function showSetupTui(
	...
	state: SetupUiState,
	surface?: { pi: ExtensionAPI; signal: AbortSignal },
	...
	if (surface) {
		try {
			const result = await openTuiSurface<SetupActionId | null>(surface.pi, ctx, {
				hostId: "@hheei/pi-hindsight:hub",
				signal: surface.signal,
				maxPending: 0,
				overlay: true,
				overlayOptions: { width: 78, maxHeight: "80%", anchor: "center", margin: 1 },
				create: ({ tui, theme, close }) => {
					const statusFacts = tabs.find((tab) => tab.id === "Status")?.facts ?? [];
					const component = createHindsightHubComponent(statusFacts, theme as ThemeLike, close);
					return {
			...
			return result.status === "closed" ? result.value : null;
		} catch (error) {
			if (error instanceof TuiSurfaceQueueFullError) {
				ctx.ui.notify("Hindsight hub is already open.", "warning");
				return null;
			}
...
export async function runHindsightSetupTui(
	...
	for (;;) {
		const config = deps.getConfig();
		const projectBankId = deps.getProjectBankId();
		const action = await showSetupTui(ctx, deps, state, surface);
