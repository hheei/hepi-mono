...
export async function loadMentalModelSources(args: {
	client: HindsightLikeClient;
	bankId: string;
	config: ResolvedConfig;
	...
	if (!args.config.mentalModels.inject || !args.client.listMentalModels) return { models: [] };
	const listed = await listModelsCached(args);
	if (listed.error) return listed;
	return {
		models: filterMentalModelsForInject(listed.models, {
