...
export function readGlobalSettingsConfig(agentDir?: string): Record<string, unknown> {
	const settings = readSettingsSection(defaultPiSettingsPaths(undefined, agentDir).globalPath);
	return settings ?? {};
}

export function readProjectSettingsConfig(cwd: string): Record<string, unknown> {
	const settings = readSettingsSection(defaultPiSettingsPaths(cwd).projectPath);
	return settings ?? {};
}
...
export function resolveConfig(cwd: string, env: NodeJS.ProcessEnv = process.env): ResolvedConfig {
	...
	const homeConfig = readGlobalSettingsConfig();
