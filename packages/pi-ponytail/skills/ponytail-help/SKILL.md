...
With `@hheei/pi-basics` loaded, open `/hepi setting` and select **Ponytail defaults**. Main-agent and subagent modes are configured separately. The same values can be written under `pi-ponytail.defaults` in the project's `.pi/settings.json`.
