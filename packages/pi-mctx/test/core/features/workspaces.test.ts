...
describe("workspace identity helpers", () => {
	...
	test("epoch fan-out bumps every member of the target workspace", () => {
		...
		try {
			...
			expect(
				db
					.prepare("SELECT project_memory_epoch FROM project_state WHERE project_path = 'git:a'")
					.get(),
			).toEqual({ project_memory_epoch: 1 });
			expect(
				db
					.prepare("SELECT project_memory_epoch FROM project_state WHERE project_path = 'git:b'")
					.get(),
			).toEqual({ project_memory_epoch: 1 });
		} finally {
			closeQuietly(db);
