...
} from "../../../src/core/features/compartment-chunk-embedding";
import {
	appendCompartments,
	getCompartments,
	replaceSessionFacts,
...
import { ensureMessagesIndexed } from "../../../src/core/features/message-index";
import {
	_resetProjectEmbeddingRegistryForTests,
	registerProjectEmbedding,
} from "../../../src/core/features/project-embedding-registry";
import { parseIdShapedQuery, unifiedSearch } from "../../../src/core/features/search";
import { initializeDatabase } from "../../../src/core/features/storage-db";
import { addNote, dismissNote, updateNote } from "../../../src/core/features/storage-notes";
import { createPrimer } from "../../../src/core/features/storage-primers";
...
describe("unifiedSearch", () => {
	...
	it("scopes note search to the current session and project notes", async () => {
		...
		const noteIds = noteResults.map((result) => result.noteId).sort((left, right) => left - right);
	...
	it("restricts results to the sources filter", async () => {
		...
		const memoryOnly = await unifiedSearch(db, "ses-sources", "/repo/project", "historian prompt", {
			memoryEnabled: true,
			embeddingEnabled: true,
			readMessages,
			embedQuery,
	...
	it("uses linear decay for message scoring so secondary hits keep signal", async () => {
		...
		const messages = results.filter(
			(r): r is Extract<(typeof results)[number], { source: "message" }> => r.source === "message",
		);
	...
	it("falls back to full semantic search when FTS finds no matches", async () => {
		...
		const results = await unifiedSearch(db, "ses-semantic", "/repo/project", "vector-only query", {
			limit: 5,
			memoryEnabled: true,
			embeddingEnabled: true,
			readMessages,
