...
import { acquireLease } from "../../../../src/core/features/dreamer/lease";
import { applyRetrospectiveLearnings } from "../../../../src/core/features/dreamer/retrospective-learnings";
import { createDreamTaskExecutor } from "../../../../src/core/features/dreamer/task-executor";
import { leaseKeyFor } from "../../../../src/core/features/dreamer/task-registry";
import type { DreamTaskRuntimeConfig } from "../../../../src/core/features/dreamer/task-scheduler";
import {
	getMemoriesByProject,
	getUnclassifiedMemoryIds,
	insertMemory,
	recordMemoryVerifications,
} from "../../../../src/core/features/memory";
import { initializeDatabase } from "../../../../src/core/features/storage-db";
import {
	ensureProjectState,
	getProjectState,
} from "../../../../src/core/features/storage-project-state";
import {
	getUserMemoryCandidates,
	insertUserMemory,
} from "../../../../src/core/features/user-memory/storage-user-memory";
...
describe("createDreamTaskExecutor — curate", () => {
	...
	test("adds the content language directive to curated prose tasks", async () => {
		...
		expect(capturedSystem).toContain("Write human-readable prose you author in: Turkish (Türkçe).");
	...
	test("runs the non-agentic XML transform and applies the manifest host-side", async () => {
		...
		const client = {
			session: {
				...
				create: vi.fn(async () => ({ data: { id: "dream-child" } })),
				prompt: vi.fn(
					async (args: { body?: { agent?: string; parts?: Array<{ text?: string }> } }) => {
						capturedPrompt = args.body?.parts?.[0]?.text ?? "";
	...
	test("direct authority.status selects rust MODULE without a prior transform", async () => {
		...
		class StatefulTimerModuleClient {
			...
			async authorityStatus() {
				...
				if (this.instanceState !== "timer-transport") throw new Error("lost transport this");
			...
			async call(args: { method: string; body: unknown }) {
				...
				const rows = (args.body as { arguments: { rows: Array<{ memory_id: number }> } }).arguments
					.rows;
		...
		expect(applyBody.arguments.rows.find((row) => row.memory_id === 10000)?.shareable).toBe(false);
	});
		...
		const client = {
			session: {
				...
				messages: vi.fn(async () => ({
					data: assistantMessages(`<cues><cue id="${memory.id}">completed anchor</cue></cues>`),
				})),
				delete: vi.fn(async () => ({})),
			},
		};
			...
			learnings: [
				{
					route: "memory",
					category: "PROJECT_RULES",
					content: "Verify provider-executed tool availability before describing it as supported.",
				},
			],
