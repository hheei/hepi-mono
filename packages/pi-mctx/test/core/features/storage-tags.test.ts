...
import {
	...
	updateTagDropMode,
	updateTagStatus,
} from "../../../src/core/features/storage-tags";
...
describe("storage-tags", () => {
	...
	describe("#given Pi fallback-tag adoption", () => {
		it("#when a fallback tag matches by fingerprint #then migrates message_id keeping tag_number", () => {
			...
			insertTag(db, "ses-1", "pi-msg-0-123-user:p0", "message", 100, 1, 0, null, 0, null, "FP-A");
		...
		it("#when fingerprint is shared by two fallback messages #then both surface (caller skips on ambiguity)", () => {
			db = makeMemoryDatabase();
			insertTag(db, "ses-1", "pi-msg-0-1-user:p0", "message", 100, 1, 0, null, 0, null, "DUP");
			insertTag(db, "ses-1", "pi-msg-1-2-user:p0", "message", 100, 2, 0, null, 0, null, "DUP");
		...
		it("#when the guarded UPDATE loses the race #then returns false and does not migrate", () => {
			db = makeMemoryDatabase();
			insertTag(db, "ses-1", "pi-msg-0-1-user:p0", "message", 100, 1, 0, null, 0, null, "FP-R");
			// Old value in the WHERE clause doesn't match (sibling already
			// migrated it) → changes === 0 → false.
			const migrated = adoptFallbackTagMessageId(db, "ses-1", 1, "pi-msg-STALE:p0", "real:p0");
