...
} from "../../../../src/core/config/schema/magic-context";
...
describe("MagicContextConfigSchema", () => {
	...
	describe("budget configuration", () => {
		it("accepts 90% execute thresholds and rejects 91%", () => {
			expect(MagicContextConfigSchema.safeParse({ execute_threshold_percentage: 90 }).success).toBe(
				true,
			);
			expect(MagicContextConfigSchema.safeParse({ execute_threshold_percentage: 91 }).success).toBe(
				false,
			);
		});
	...
	describe("valid config", () => {
		...
		it.skip("accepts optional auto_update user preference", () => {
			MagicContextConfigSchema.parse({ auto_update: false });
			MagicContextConfigSchema.parse({ auto_update: true });
		});
	...
	describe("validation", () => {
		...
		it("rejects historian_timeout_ms below minimum", () => {
			expect(() => MagicContextConfigSchema.parse({ historian_timeout_ms: 59_999 })).toThrow();
		});
