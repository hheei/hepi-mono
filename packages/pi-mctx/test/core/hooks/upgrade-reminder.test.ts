...
import {
	...
	updateSessionMeta,
} from "../../../src/core/features/storage-meta-session";
import {
...
} from "../../../src/core/hooks/upgrade-reminder";
...
describe("E5 upgrade reminder", () => {
	...
	it("fires at most three reminders per session even after every cooldown", async () => {
		...
		for (
			...
			expect(getOrCreateSessionMeta(db, "ses-capped").upgradeReminderCount).toBe(expectedCount);
