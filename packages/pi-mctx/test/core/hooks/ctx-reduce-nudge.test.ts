...
} from "../../../src/core/hooks/ctx-reduce-nudge";
...
describe("shouldTriggerChannel2 — ceiling (reclaimable ≥ usable/3)", () => {
	it("fires when reclaimable is at least a third of the usable working range", () => {
		...
		expect(shouldTriggerChannel2({ reclaimableTokens: 30_000, usableTokens: 90_000 })).toBe(true);
	});
	it("the AFT regression: 54k reclaimable on a wide 1M session does NOT fire", () => {
		...
		expect(shouldTriggerChannel2({ reclaimableTokens: 54_000, usableTokens: 300_000 })).toBe(false);
	});
	it("the SAME 54k on a tight 120k usable session DOES fire (size-relative)", () => {
		...
		expect(shouldTriggerChannel2({ reclaimableTokens: 54_000, usableTokens: 120_000 })).toBe(true);
	});
