...
import { CLASSIFY_SYSTEM_PROMPT } from "../../../src/core/features/dreamer/classify-prompt";
import { MAP_MEMORIES_SYSTEM_PROMPT } from "../../../src/core/features/dreamer/map-memories-prompt";
import {
...
} from "../../../src/core/features/dreamer/task-prompts";
import { VERIFY_SYSTEM_PROMPT } from "../../../src/core/features/dreamer/verify-prompt";
import { MIGRATION_SYSTEM_PROMPT } from "../../../src/core/features/memory/memory-migration";
import { SIDEKICK_SYSTEM_PROMPT } from "../../../src/core/features/sidekick/core";
import { SMART_NOTE_COMPILER_SYSTEM_PROMPT } from "../../../src/core/features/smart-notes/compiler-prompt";
import {
	closeDatabase,
...
} from "../../../src/core/hooks/compartment-prompt";
import {
	clearCtxReduceAvailability,
	resolveCtxReduceAvailabilityFromMessages,
} from "../../../src/core/hooks/ctx-reduce-availability";
import {
	createSystemPromptHashHandler,
	isMagicContextInternalAgent,
} from "../../../src/core/hooks/system-prompt-hash";
...
describe("system-prompt-hash v2 system prompt contents", () => {
	...
	it("injects language guidance and stabilizes after the config changes once", async () => {
		...
		expect(changed.join("\n")).toContain("Use Turkish (Türkçe) for your natural-language replies");
		expect(historyRefreshSessions.has(sessionId)).toBe(true);
		expect(pendingMaterializationSessions.has(sessionId)).toBe(true);
...
describe("system-prompt-hash skips Magic Context internal child agents", () => {
	...
	const DREAMER_BASE_HEAD = "You are a background maintenance agent for the magic-context system,";
...
describe("system-prompt-hash honors per-agent opt-out (issue #53)", () => {
	...
	it("matches multiple skip signatures (any one match opts the agent out)", async () => {
		...
		const { handler } = buildHandler({
			injectionSkipSignatures: ["<!-- magic-context: skip -->", "I AM A TINY SPECIALIZED AGENT"],
		});
