...
import { addNote } from "../../../src/core/features/storage-notes";
import {
	clearNoteNudgeState,
	...
	onNoteTrigger,
	peekNoteNudgeText,
} from "../../../src/core/hooks/note-nudger";
...
describe("note-nudger", () => {
	...
	it("returns null when no notes exist even if triggered", () => {
		...
		onNoteTrigger(db, "ses-empty", "historian_complete");
	...
	it("still delivers when a new note arrives after the last mctx_note(read)", () => {
		...
		).run("session", "active", "New note after last read.", "ses-new-activity", newerAt, newerAt);
		...
		expect(peekNoteNudgeText(db, "ses-new-activity", "u-2")).toContain("You have 2 deferred notes");
	});
	...
	it("clearNoteNudgeTriggerOnly preserves delivered anchors", () => {
		...
		onNoteTrigger(db, "ses-trigger-only", "historian_complete");
