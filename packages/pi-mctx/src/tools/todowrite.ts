 ...
 * # Why this exists
 *
 * Magic Context provides a built-in `todowrite` because Pi treats todo
 * management as an extension concern. It validates `{ todos: [...] }`, returns
 * a pretty-printed JSON acknowledgement, and lets the message-end capture path
 * snapshot the args into `session_meta.last_todo_state`.
 */
...
export function createTodowriteTool(): ToolDefinition<typeof TodowriteParams> {
	return {
		...
		async execute(
			...
			// Output is pretty-printed JSON of the full todo list.
