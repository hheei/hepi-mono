...
import { clearCompressionDepthRange } from "../features/compression-depth-storage";
...
export function snapRangeToCompartments(
	...
	if (compartments.length === 0) {
		return {
			error:
				"No compartments exist yet for this session. Run `/ctx-recomp` (full) first, then use partial recomp to refine specific ranges.",
		};
	}
...
export async function executePartialRecompInternal(
	...
	try {
		...
		function promoteFinal(): { compartmentCount: number; lastEndMessage: number } | null {
			...
			// mctx_search semantic results. Gated on memory-enabled, distinct from
