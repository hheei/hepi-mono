...
import { openDatabase } from "../features/storage";
import type { SubagentKind } from "../features/storage-subagent-invocations";
import {
...
// (<project>/.pi/magic-context/historian/) so they sit inside the
// project boundary Pi's native tools already trust AND so users
...
function extractLatestHistorianReasoning(messages: unknown): string | null {
	...
	const latest = messages
		...
		.sort((left, right) => historianMessageCreatedAt(right) - historianMessageCreatedAt(left))[0];
...
async function runHistorianPrompt(args: {
	...
	const recordInvocation = (params: {
		...
		return recordChildInvocation({
			...
			parentSessionId,
			subagent:
				agentId === HISTORIAN_EDITOR_AGENT ? "historian_editor" : (subagentKind ?? "historian"),
			startedAt,
