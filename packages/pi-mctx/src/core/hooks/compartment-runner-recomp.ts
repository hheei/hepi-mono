...
} from "../features/compartment-storage";
import { clearCompressionDepth } from "../features/compression-depth-storage";
import { resolveProjectIdentity } from "../features/memory/project-identity";
import { appendM0Mutation } from "../features/storage";
import {
	recordHistorianRun,
	summarizeImportance,
	tallyFactsByCategory,
} from "../features/storage-historian-runs";
import { clearCachedM0M1, updateSessionMeta } from "../features/storage-meta";
...
export async function executeContextRecompInternal(deps: CompartmentRunnerDeps): Promise<string> {
	...
	try {
		...
		async function promoteAndFinalize(reason: string): Promise<string | null> {
			...
			// embeddings and vanish from mctx_search semantic results. Embedding is
		...
		// → they vanish from mctx_search semantic results. Gated on memory-enabled,
