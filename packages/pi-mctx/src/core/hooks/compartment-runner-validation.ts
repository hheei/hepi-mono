...
export function validateHistorianOutput(
	...
	const mapped = mapParsedCompartmentsToChunk(
		parsed.compartments.map((compartment) => ({
			...
			...(compartment.importance === undefined ? {} : { importance: compartment.importance }),
			...(compartment.episodeType === undefined ? {} : { episodeType: compartment.episodeType }),
		})),
		chunk,
		sequenceOffset,
	);
 ...
 *     the last error and the actionable next step (check `/ext-settings`).
 *
...
export function buildHistorianFailureNotice(failureCount: number, lastError: string): string {
	if (failureCount >= HISTORIAN_PERSISTENT_FAILURE_THRESHOLD) {
		return [
			...
			"",
			"Check your historian model in `/ext-settings`, then reload or restart. Your conversation keeps working normally in the meantime — this only affects how older history is summarized.",
		].join("\n");
	}
	return [
		"## Magic Context",
