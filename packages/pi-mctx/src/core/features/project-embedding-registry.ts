...
function createProvider(
	...
	if (config.provider === "openai-compatible") {
		const options = {
			...
			...(config.query_input_type !== undefined ? { queryInputType: config.query_input_type } : {}),
			...(config.truncate !== undefined ? { truncate: config.truncate } : {}),
			...(config.max_input_tokens !== undefined ? { maxInputTokens: config.max_input_tokens } : {}),
		};
		...
		for (const { scope, enabled, currentModelId } of scopes) {
			...
			for (const modelId of staleModelsForScope(
				...
				if (!hasStaleEmbeddingRows(db, scope, projectIdentity, modelId)) {
					result.trackingRowsDeleted += deleteTracking.run(projectIdentity, scope, modelId).changes;
				} else if (deleted === 0) {
...
function countShadowBackfillMissing(
	...
	return (db.prepare(`SELECT COUNT(*) AS count FROM (${sql})`).get(...params) as { count: number })
		.count;
}
...
export async function flushShadowEmbeddingBacklog(
	...
	for (;;) {
		...
		startShadowWorker();
		if (shadowWorker == null) await new Promise((resolve) => setTimeout(resolve, 10));
		await onSettled?.();
	}
}
...
export async function embedItemsForProject(
	...
	const tableEpoch =
		typeof rawConfig.synapse_table_epoch === "number" ? rawConfig.synapse_table_epoch : undefined;
	const isSynapse =
		...
		for (let attempt = 0; attempt < EMBED_SLICE_RETRY_ATTEMPTS; attempt++) {
			...
			if (result) {
				for (const item of slice) {
					...
					const rows: SaveCompartmentChunkEmbeddingInput[] = item.windows.map((window, index) => ({
						compartmentId: item.candidate.id,
					...
					enqueueShadowEmbeddingItems(projectIdentity, "chunk", [String(item.candidate.id)]);
				}
			}
...
export async function embedSessionCompartmentChunks(
	...
	if (providerDown || failedIds.length > 0) {
		const remaining = Math.max(
			0,
			countUnembeddedSessionCompartments(db, projectIdentity, sessionId, snapshot.chunkModelId) -
				skipIds.length,
		);
		if (remaining > 0) {
...
export async function sweepAllRegisteredProjects(
	...
	try {
		for (const projectIdentity of projectRegistrations.keys()) {
			...
			if (Date.now() < deadline) {
				chunks = await drainCompartmentChunkBacklogForProject(db, projectIdentity, deadline);
