...
export function loadPersistedUsage(db: Database, sessionId: string): PersistedUsageState | null {
	...
	return {
		...
		lastUsageContextLimit:
			typeof result.last_usage_context_limit === "number" ? result.last_usage_context_limit : 0,
	};
}
...
export function recordHistorianDrainFailure(db: Database, sessionId: string, now?: number): void {
	...
	db.transaction(() => {
		ensureSessionMetaRow(db, sessionId);
		db.prepare("UPDATE session_meta SET historian_drain_failure_at = ? WHERE session_id = ?").run(
			ts,
			sessionId,
		);
	})();
...
export function clearHistorianDrainFailure(db: Database, sessionId: string): void {
	db.transaction(() => {
		ensureSessionMetaRow(db, sessionId);
		db.prepare("UPDATE session_meta SET historian_drain_failure_at = 0 WHERE session_id = ?").run(
			sessionId,
		);
	})();
}
...
export function setEmergencyDropSample(db: Database, sessionId: string, inputSample: number): void {
	db.transaction(() => {
		ensureSessionMetaRow(db, sessionId);
		db.prepare("UPDATE session_meta SET last_emergency_input_sample = ? WHERE session_id = ?").run(
			Math.max(0, Math.round(inputSample)),
			sessionId,
		);
	})();
}
...
export function clearEmergencyDropSample(db: Database, sessionId: string): void {
	db.transaction(() => {
		ensureSessionMetaRow(db, sessionId);
		db.prepare("UPDATE session_meta SET last_emergency_input_sample = 0 WHERE session_id = ?").run(
			sessionId,
		);
	})();
}
...
export function getChannel2NudgeClaim(db: Database, sessionId: string): Channel2NudgeClaim {
	...
	const state: Channel2NudgeState =
		rawState === "pending" || rawState === "claimed" || rawState === "delivered" ? rawState : "";
	return {
		state,
		claimedAt:
			typeof result?.channel2_nudge_claimed_at === "number" ? result.channel2_nudge_claimed_at : 0,
		claimToken:
...
export function getPersistedNoteNudge(db: Database, sessionId: string): PersistedNoteNudge {
	...
	return {
		...
		triggerMessageId:
			result.note_nudge_trigger_message_id.length > 0 ? result.note_nudge_trigger_message_id : null,
		stickyText: result.note_nudge_sticky_text.length > 0 ? result.note_nudge_sticky_text : null,
		stickyMessageId:
			result.note_nudge_sticky_message_id.length > 0 ? result.note_nudge_sticky_message_id : null,
	};
}
...
function casUpdateJsonArrayColumn<T>(
	...
	for (let attempt = 0; attempt < CAS_RETRY_LIMIT; attempt += 1) {
		...
		const result = db
			.prepare(`UPDATE session_meta SET ${column} = ? WHERE session_id = ? AND ${column} IS ?`)
...
export function getOverflowState(
	...
	const limit =
		typeof result.detected_context_limit === "number" && result.detected_context_limit > 0
			...
			: 0;
	const modelMatches =
		limit > 0 && requestedModelKey && storedModelKey ? requestedModelKey === storedModelKey : true;
	const needs =
