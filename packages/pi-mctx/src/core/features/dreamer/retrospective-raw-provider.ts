...
export interface RetrospectiveRawProvider {
    listProjectSessions(projectIdentity: string):
...
function compareMessages(left: RetrospectiveRawMessage, right: RetrospectiveRawMessage): number {
    return left.ts - right.ts || left.sessionId.localeCompare(right.sessionId) || left.ordinal - right.ordinal;
}
...
export async function readRetrospectiveScanWindow(
    ...
    const maxMessages = Math.max(1, Math.floor(options.maxMessagesPerRun ?? DEFAULT_MAX_MESSAGES_PER_RUN));
    const capPerSession = Math.max(1, Math.floor(options.capPerSession ?? DEFAULT_CAP_PER_SESSION));
    const maxSessions = Math.max(1, Math.floor(options.maxSessionsPerRun ?? DEFAULT_MAX_SESSIONS_PER_RUN));
