...
export interface DreamTaskExecutorDeps {
	...
	primerRawProviderFactory?:
		| ((sessionId: string) => Promise<RawMessageProvider | null> | RawMessageProvider | null)
		| undefined;
	...
	return async (
		...
		try {
			if (config.task === "compress-cues") {
				...
				if (moduleRoute) {
					const reason = "compress-cues parked: MODULE memory authority has no cue write facade";
...
async function runRetrospectiveTask(
	...
	const postWatermarkOrdinals = new Set(
		userMessages.filter((message) => message.ts > watermarkMs).map((message) => message.ordinal),
	);
	...
	try {
		...
		const deepenRun = await runChildTurn(
			withContentLanguageDirective(RETROSPECTIVE_SYSTEM_PROMPT, config.language ?? deps.language, {
				retrospective: true,
			}),
