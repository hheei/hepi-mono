...
import { type GitCommitSearchHit, searchGitCommitsSync } from "./git-commits/search-git-commits";
...
async function getSemanticScores(args: {
	...
	if (!args.workspace?.isWorkspaced) {
		...
		for (const memory of args.memories) {
			...
			semanticScores.set(
				memory.id,
				normalizeCosineScore(cosineSimilarity(args.queryEmbedding, memoryEmbedding.embedding)),
			);
		}
...
function searchCompartmentChunks(args: {
	...
	if (!args.queryEmbedding || args.limit <= 0 || !args.modelId || args.modelId === "off") return [];
	const cutoff = args.maxOrdinal != null && args.maxOrdinal >= 0 ? args.maxOrdinal : null;
	...
	const byCompartment = new Map<number, { row: StoredCompartmentChunkEmbedding; score: number }>();
	for (const row of rows) {
...
function mergeMessageAndCompartmentResults(args: {
	...
	const ranked = [...fused.values()]
		.sort((left, right) =>
			right.score !== left.score ? right.score - left.score : left.tieOrdinal - right.tieOrdinal,
		)
		.slice(0, args.limit);
		...
		.map((primer) => {
			const semantic =
				...
				primer.questionEmbeddingModelId === args.queryModelId
					? normalizeCosineScore(cosineSimilarity(args.queryEmbedding, primer.questionEmbedding))
					: 0;
...
export async function unifiedSearch(
	...
	const chunkModelId =
		queryContract?.chunkModelId ?? options.chunkModelIdOverride ?? embeddingSnapshot?.chunkModelId;
