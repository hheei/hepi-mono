 ...
 * mctx_reduce modes so it sets the frame before the agent reads anything that could
...
/** Closer for sessions where mctx_reduce is callable — the agent has an active housekeeping role. */
...
/** Closer for sessions where mctx_reduce is unavailable — context is managed fully automatically. */
...
/**
 * Shared `mctx_note` guidance for both intro variants. Generalizes two observed
...
/** Intro when mctx_reduce is unavailable — no drop guidance, no mctx_reduce
 *  references, and no tag system description. When the session's tool allow-list
 *  denies mctx_reduce, transform.ts skips §N§ prefix injection entirely, so the
 ...
 * mctx_reduce mechanics. The `## Magic Context` marker is still present for
 * injection idempotency (system-prompt-hash.ts gates on it).
 */
const SUBAGENT_REDUCE_INTRO = (
	protectedTags: number,
): string => `Messages and tool outputs are tagged with §N§ identifiers (e.g., §1§, §42§).
Use \`mctx_reduce\` to drop tool outputs you have already finished with, keeping your working context lean. Syntax: "3-5", "1,2,9", or "1-5,8,12-15". The last ${protectedTags} tags are protected.
