...
// navigation/read/search tools only — NO write/edit/bash/mctx_memory/mctx_note —
// because it runs an unsupervised, scheduled, weak-model agentic loop. A
// mctx_memory mutation here would bump the project memory epoch and bust m[0],
...
// primer investigator (NO write/edit/bash/mctx_memory), but deliberately WITHOUT
// mctx_search — these tasks check memories against the CURRENT local source, not
...
// find) + aft navigation, but deliberately NO mctx_memory/mctx_search/mctx_note —
 ...
 *  hygiene). Curate edits the memory store through mctx_memory and never reads
