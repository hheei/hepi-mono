...
import { byteSize, prependTag, stripTagPrefix } from "../hooks/tag-content-primitives";
...
export interface TagTranscriptOptions {
	...
	onTiming?:
		| ((phase: "identity" | "prefix" | "targets" | "tokenCounting", elapsedMs: number) => void)
		| undefined;
}
		...
		for (let partIndex = 0; partIndex < parts.length; partIndex += 1) {
			...
			if (part.kind === "text") {
				...
				tagTextPart({
					...
					...optionalTextTagArgs(timing, options.textIdentitySourceCache, options.textTokenCache),
				});
				textOrdinal += 1;
				continue;
			}
					...
					if (canReuseIdentity) {
						...
						if (part.kind === "tool_result") {
							...
							applyGrownToolResultAccounting({
								...
								part,
								text,
								...optionalAccountingArgs(timing, options.toolTokenCache, tokenCacheKey),
							});
						}
...
function tagTextPart(args: TagTextPartArgs): void {
	...
	const tagId = args.tagger.assignTag(
		...
		() => {
			...
			const tokenCount =
				cached?.text === text ? cached.tokenCount : estimateTagTextTokens(stripTagPrefix(text));
			if (cached?.text !== text) {
