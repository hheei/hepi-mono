...
function stopRetry(error: unknown, signal: AbortSignal | undefined): boolean {
    ...
    return error.name === "AbortError" || error.name === "TimeoutError" || /timed out|timeout|prompt is too long|context overflow/i.test(error.message);
}
...
async function promptAttempt(client: PromptClient, args: Record<string, unknown>, options: RetryOptions): Promise<void> {
    ...
    try {
        ...
        await (timeout ? Promise.race([prompt, timeout]) : prompt);
    } catch (error) {
        if (options.signal?.aborted) throw abortError();
        throw error;
    } finally {
...
export async function promptSyncWithModelSuggestionRetry(
    ...
    const queue = [undefined, ...(options.fallbackModels ?? []).filter((model) => Boolean(modelParts(model).model))] as (string | undefined)[];
    ...
    for (let index = 0; index < queue.length; index += 1) {
        ...
        try {
            ...
            lastError = error;
            if (stopRetry(error, options.signal)) throw error instanceof Error && options.signal?.aborted ? abortError() : error;
...
export async function promptSyncWithValidatedOutputRetry<Output, Validated>(
    ...
    for (let index = 0; index < queue.length; index += 1) {
        ...
        try {
            await promptAttempt(client, { ...args, body: { ...args.body, ...modelParts(model) } }, options);
            const output = await options.fetchOutput();
            return { output, validated: options.validateOutput(output, { label }) };
        } catch (error) {
            lastError = error;
            if (firstValidationError === undefined && error instanceof Error && /empty output/.test(error.message)) firstValidationError = error;
            if (stopRetry(error, options.signal)) throw error instanceof Error && options.signal?.aborted ? abortError() : error;
        }
    }
