...
async function runHistorianSubagentWithTransientRetries(args: {
	...
	sessionId: string;
	passLabel: string;
	retryBackoffMs?: ((retryIndex: number) => number) | undefined;
...
export async function runPiHistorian(deps: PiHistorianDeps): Promise<void> {
	...
	try {
		...
		await withRawMessageProvider(sessionId, provider, async () => {
			...
			// Compaction remains available with memory off, but fact dedup
			// against <project-memory> is only valid when that store is enabled.
			...
			if (embeddingActive) {
				...
				for (const [i, c] of newCompartments.entries()) {
