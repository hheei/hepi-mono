...
HEPI fork of [`@narumitw/pi-starship`](https://github.com/narumiruna/pi-extensions/tree/main/extensions/pi-starship), imported from upstream v0.22.0 at commit [`0606337`](https://github.com/narumiruna/pi-extensions/commit/0606337fc258b38331f025f6e63e6a882f181686). Upstream copyright, MIT license, and Starship notices remain in [`LICENSE`](./LICENSE) and [`NOTICES.md`](./NOTICES.md).
...
> The unscoped npm package `pi-starship` delegates to the Starship binary. This package renders Pi-specific modules natively.
...
Run this fork in isolation:

```bash
bun run pi:dev -- starship
```

Do not load this together with Pi Basics while its native statusbar is enabled: both own the Pi footer.
