 ...
 *     not OpenCode's `parts: unknown[]`. Pi performs its own boundary trim
 *     and writes the rendered block directly into the Pi message shape.
 ...
 *
 * Cache safety:
 *   - On defer passes (`isCacheBusting=false`) the cached rendered block is
 ...
 *
 * Mirrors the prefix trim used by the shared compartment injection logic.
 *
