...
For HEPI behavior, then read the owning module under `packages/hepi-basics/src`, `packages/hepi-tools/src`, `packages/hepi-skills/src`, or `packages/hepi-mono/src`. Shared contracts belong in Basics `core`; feature state machines and parsers stay in their feature module.
...
- Keep aggregate boundaries intact: `hepi-basics`, `hepi-tools`, `hepi-skills`, and `hepi-mono` publish bundled `dist/extension.js`; static skills and themes are package resources, not runtime imports.
