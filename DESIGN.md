...
- In `native` Edit Mode, `edit` and `write` keep Pi's execution semantics but render through `ToolTui` like the other `pi-ext-tools` tools. They must not show a second built-in header inside the body. `edit` shows only its diff or error body inside the shared rails, while `write` shows only its file-content preview or error body inside the shared rails. Successful `write` results retain the same preview body after completion rather than collapsing to a header-only success row.
- Native `write` keeps Pi's syntax highlighting, incremental partial preview, and `10 lines + expand` preview rule inside the shared tool body. Its typed footer is `X bytes · Y lines · duration`. Native `edit` keeps Pi's preview/result diff semantics inside the shared tool body. Its typed footer is `N replacements · duration`.
