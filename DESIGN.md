...
- `pi-mctx` 的 `mctx_search` / `mctx_memory` / `mctx_note` / `mctx_expand` / `mctx_reduce` 使用同一共享 `ToolTui`。Header 显示 tool label 与 request summary（query、action、range、drop）；call-phase 只有 header；result body 是 model-visible text，无独立 typed footer。
- AgentMemory automatic recall 仅在 interactive TUI 的 Context Projection 成功 publish 后显示 ext-core-managed `aboveEditor` widget。标题使用 `accent`，source preview 使用 terminal `text`，额外计数使用 `dim`；最多四个 event、每个只显示第一条 source、单条 source 最多 240 characters，并通过 `truncateToWidth` 保证 narrow/wide cell-width safe。Headless/RPC 不挂载 widget；`/ctx-status` 最多显示三个经过 control-character stripping、whitespace collapse 的 recent recall preview。
